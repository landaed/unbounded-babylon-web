...
void main(void) {
    ...
    float a = iqnoise(groundScale*(worldPos.xz+vec2(groundOffset)), irregular, smoothness)*groundHeight;
    float b = iqnoise(groundScale*(worldPos.xz-vec2(1,0)+vec2(groundOffset)), irregular, smoothness)*groundHeight;
    float c = iqnoise(groundScale*(worldPos.xz-vec2(0,1)+vec2(groundOffset)), irregular, smoothness)*groundHeight;

    vec3 A = vec3(worldPos.x, worldPos.y-a, worldPos.z);
    vec3 B = vec3(worldPos.x-1., worldPos.y-b, worldPos.z);
    vec3 C = vec3(worldPos.x,worldPos.y-c, worldPos.z-1.);
    worldPos.y -= iqnoise(groundScale*(worldPos.xz+vec2(groundOffset)), irregular, smoothness)*groundHeight;
    ...
    vPosition = p.xyz;
        ...
        //vNormal = normalize(normalWorld * normalUpdated);
